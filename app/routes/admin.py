...
@router.post("/upload-students", status_code=200)
async def upload_students(
    ...
    Returns:
        Upload result with counts and errors
    """
    try:
        # Validate file type
        ...
        # Process upload
        successful, failed, errors = await upload_service.process_excel_upload(
...
@router.patch("/student/{user_id}/category", status_code=200)
async def update_student_category(
    ...
    try:
        ...
        user_service.update_user(user_id, {"category": category.upper()})
        ...
        logger.error(f"Error updating student category: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error updating student category",
        )
